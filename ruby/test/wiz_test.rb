...
module TinyMud
    class TestWiz < Test::Unit::TestCase
		...
		def test_do_teleport
			...
			Interface.expects(:do_notify).with(another_bob, Phrasebook.lookup('seen-in-mist', "bob")).in_sequence(notify)
			...
			Interface.expects(:do_notify).with(bob, Phrasebook.lookup('seen-in-mist', "Wizard")).in_sequence(notify)
			Interface.expects(:do_notify).with(another_bob, Phrasebook.lookup('seen-in-mist', "Wizard")).in_sequence(notify)
			Interface.expects(:do_notify).with(wizard, Phrasebook.lookup('contents')).in_sequence(notify)
			Interface.expects(:do_notify).with(wizard, 'bob(#3)').in_sequence(notify)
